...
The system is built to watch a single confined area, a safe holding valuables, an office, or a similar space, and give the owner a grace window to disarm before the siren fires. When armed, the system monitors a room with a PIR motion sensor. Sustained motion (roughly five consecutive seconds of activity) triggers a 20 second countdown (shown on a 14-segment display with an accelerating warning LED and audible beeps). If the countdown expires, the alarm sounds through a speaker while an 8×8 LED matrix flashes red. The system can be disarmed four ways:
...
Arming mirrors this: enter the code on the keypad or press the dashboard's Arm button, or press in the Zen cape joystick, a deliberately password free on/off toggle kept for demos.
...
Video transcoding and QR scanning were deliberately offloaded from the BeagleBone to the host PC for performance, the reason two Node.js servers exist. The project is split across three cooperating programs connected by UDP over an Ethernet over USB link (BeagleBone `192.168.7.2` ↔ host PC `192.168.7.1`):
...
│   ├─ ALSA audio (warning beeps/alarm siren), port controlled │
...
**C application (`main.c` + modules).** Each hardware subsystem runs on its own POSIX thread and coordinates through a shared `Alarm` struct. The core logic thread evaluates the state machine once per second; the motion thread integrates PIR readings (incrementing on motion, decaying otherwise) so only sustained motion escalates the state. The webcam thread streams 720×720 H.264 frames over UDP and optionally runs a lightweight motion detector that computes the sum of absolute differences on a sampled band of each frame (whole frame diffing proved too slow on the ARM target).
...
| `1` armed | Watching | PIR events accumulate; ~5 s of sustained motion → alerted |
...
- PIR motion sensor and standalone status LED
- External 8×8 LED matrix (HT16K33, I2C)
- USB webcam (720p/1080p) and USB numeric keypad
- External speakers driven via ALSA for warning/alarm WAV playback; volume controlled by the potentiometer

## Design Notes
- **Matrix keypad → numpad pivot.** The original plan was a breadboard wired matrix keypad, but after extensive debugging a few keys always failed. The feature was replaced with a USB numpad, and QR code disarming was added to compensate, turning a hardware dead end into the project's most distinctive feature.
- **Snapshot instead of recording.** Saving full motion recordings to disk was attempted but shelved; the pipeline persists the most recent frame snapshot instead.
- **LAN scoped control.** Outbound alerts (SMS/MMS) reach the owner anywhere, but inbound control is limited to the local network; exposing the dashboard publicly for remote arm/disarm was planned but not completed.
...
- **High stream latency (several seconds)** — seen when the stream server runs inside a resource constrained VM; it disappears on machines with adequate CPU/RAM. Run the host server natively or increase the VM's allocation.
