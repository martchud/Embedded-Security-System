...
{	
	//Wipe playbackBuffer
	memset( playbackBuffer, 0, sizeof(short)*playbackBufferSize );
	...
	for(int i = 0 ; i < MAX_SOUND_BITES; i++){
		...
		//preload values into local variables
		int offset = soundBites[i].location;
		int numSamples = soundBites[i].pSound->numSamples;

		for(int j = 0 ; j < size; j++){
			//entire sample played
			if(offset == numSamples){
				soundBites[i].pSound = NULL;
				soundBites[i].location = 0;
				break;
			}
			//add until buffer filled or entire sample is played
			else{
				int currentValue = (playbackBuffer[j] + soundBites[i].pSound->pData[offset]);
				short newVal;
				if(currentValue < SHRT_MIN){
					//underflow
					newVal = SHRT_MIN;
				}else if(currentValue > SHRT_MAX){
					//overflow
					newVal = SHRT_MAX;
				}else{
					newVal = currentValue;					
				}
				//update playBackBuffer and soundBites
				playbackBuffer[j] = newVal;
				offset++;
