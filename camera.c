 ...
 *
 * [+] Added frame data packet sending as shown here: https://opencoursehub.cs.sfu.ca/bfraser/grav-cms/ensc351/links/files/2022-student-howtos/StreamingWebcamFromBeagleBoneToNodeJSServer.pdf
 *
 * [+] Modified to detect motion from analyzing frame data
 */
...
#define SAMPLE_BUFF_SIZE 590 //Only sample the edge (save processing)
...
static uint8_t prevBuf[SAMPLE_BUFF_SIZE];

static pthread_t capturethread;
static bool killthread = false;
static bool detect_motion = false;
...
static void mainloop(void)
{
	...
	count = frame_count;
        ...
        while ( ((count-- > 0) || loopIsInfinite) && !killthread) {
                for (;;) {
                        fd_set fds;
                        struct timeval tv;
                        int r;
